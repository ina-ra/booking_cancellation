...
```mermaid
  ...
  D --> E["Базовая очистка + простые признаки"]
  E --> F["Train/Test split"]
  F --> G["Простая модель"]
  G --> H["Оценка метрик (ROC-AUC)"]
  H --> I["Предсказания"]
```
Блок-схема MVP отражает полный ML pipeline от данных до формирования бизнес-результата::

```mermaid
  flowchart TB
  C[MVP] --> D["Исторические данные"]
  ...
  K --> L["Формирование выходного датасета"]
```
