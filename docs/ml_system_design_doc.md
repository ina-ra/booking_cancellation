...
flowchart TD
  B[Бейзлайн] --> D{Исторические данные} --> E{Предобработка (+ Feature Engineering)} --> F{Train/Test split} --> G{Простая модель} --> H{Оценка метрик (ROC-AUC)} --> I{Предсказания}
  C[MVP] --> D --> E --> F --> J{Обучение моделей-кандидатов (CatBoost, LightGBM, XGBoost, LR) и выбор лучшей --> K{Скоринг + ранжирование + выделение high-risk сегмента (Top-30%)} --> L{Output}
